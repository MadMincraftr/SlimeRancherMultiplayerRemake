...
namespace kcp2k
{
    public class KcpClient : KcpPeer
    {
        ...
        {
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log(address);
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("connect start");
            if (connected)
            {
                Log.Warning("[KCP] Client: already connected!");
                return;
            }
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("resolve host name");
            ...
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("reset config");
            ...
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("create remote end point");

            // create socket
            remoteEndPoint = new IPEndPoint(addresses[0], port);
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("create socket");
            socket = new Socket(remoteEndPoint.AddressFamily, SocketType.Dgram, ProtocolType.Udp);
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("set active");
            active = true;
            ...
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("socket blocking false");
            socket.Blocking = false;

            // configure buffer sizes
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("configure stuff");
            ...
            if (SRMP.SRMLConfig.DEBUG_LOG) SRMP.SRMP.Log("true connect");
